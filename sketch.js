...
background("skyblue")
 obstacles();
  ...
  if (gameState===play){
    ...
    if((touches.length > 0 || keyDown("space"))&&monkey.y >= 450) {
      monkey.velocityY = -17; 
    }
  ...
  if (gameState===END){
   FoodGroup.setVelocityXEach(0)
    ...
    bg1.velocityX = 0
    monkey.scale = 0.12
   reset = createSprite(width/1.1,50)
  ...
  stroke("black")
    textSize(30)
    text("press R to reset or press that reset button",200,300)
    fill("blue")
    textSize(15)
    text("G A M E  O V E R😛 😛",250,350)
    
    if (touches.length || keyDown("R") || mousePressedOver(reset)){
...
function obstacles(){
 if (frameCount % 200===0){
  obstacle = createSprite(width/1,530)
...
function banana(){
if (frameCount % 300===0){
  var food = createSprite(width/1,300)
